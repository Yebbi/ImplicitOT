...
def load_data():
    return np.load('/home/yesom/Codes/Implicit_OT/data/power/data.npy')
...
def load_data_normalised():
    ...
    ## Normalize via statics
    # mu = data.mean(axis=0)
    # s = data.std(axis=0)
    
    ## Normalize via scale
    s = (np.max(data,axis=0) - np.min(data,axis=0))/2
    mu = (np.max(data,axis=0) + np.min(data,axis=0))/2
